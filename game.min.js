...
// lib/game/data/music-manager.js
ig.baked=true;ig.module('game.data.music-manager').requires('impact.impact').defines(function(){ig.musicManager=ig.Class.extend({musicTracks:{},staticInstantiate:function(ignore){if(ig.ResourceManager==null){return null;}
return ig.ResourceManager.instance;},init:function(){ig.ResourceManager.instance=this;},addMusicTrack:function(fileName,trackName){this.musicTracks[trackName]=fileName;},getMusicTrack:function(trackName){return this.musicTracks[trackName];}});});

// lib/game/scenes/ingame.js
ig.baked=true;ig.module('game.scenes.ingame').requires('impact.game').defines(function(){'use strict';ig.Ingame=ig.Game.extend({clearColor:"#aec440",counter:0,points:new ig.Font('media/gfx/fat_kid.png'),ingameMusic:new ig.Sound('media/snd/msc/ingame.*'),init:function(){this.initMusic();this.infiniteLevel=new ig.InfiniteLevel([Level01,Level02,Level03]);ig.game.spawnEntity(ig.EntityPlayer,ig.system.width*0.5,ig.system.height*0.5);},initMusic:function(){if(ig.music.tracks.length>=1){ig.music.tracks.shift();}
ig.music.add(this.ingameMusic,'ingame');ig.music.play('ingame');},update:function(){this.infiniteLevel.update();this.parent();this.handleInput();this.camera();},camera:function(){this.counter+=3*ig.system.tick;if(this.counter>6){var i=(Math.floor(5*Math.random())+1)*2;this.counter=0;do{ig.game.spawnEntity(ig.EntityBubble,ig.game.player.pos.x*((5*Math.random())+i),ig.system.height*(Math.random()*i)+1);}while(i=i-1)
ig.game.spawnEntity(ig.EntityAnchor,ig.system.width*((2*Math.random())+1),-((22*Math.random())+40));}
this.screen.x=this.player.pos.x-ig.system.width*0.025;this.screen.y=15+Math.sin(this.counter);},handleInput:function(){if(ig.input.pressed('action_01')||ig.input.pressed('action_02')){ig.system.setGame(ig.Title);return;}},draw:function(){this.parent();this.points.draw(Math.floor(ig.game.player.distance)+'.MTS',160,0,ig.Font.ALIGN.RIGHT);}});});
...
ig.baked=true;ig.module('game.entities.player').requires('impact.entity').defines(function(){'use strict';ig.EntityPlayer=ig.Entity.extend({size:{x:38,y:16},offset:{x:0,y:3},accel:200,vel:{x:100,y:0},maxVel:{x:200,y:200},friction:{x:0,y:1000},type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,collides:ig.Entity.COLLIDES.FIXED,animSheet:new ig.AnimationSheet('media/gfx/spr/shark.png',40,20),health:3,maxHealth:3,distance:0,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('swim',0.12,[0,1,2,1,0]);ig.game.player=this;this.distance=0;},update:function(){var accel=this.accel;if(ig.input.state('up')){this.vel.y=-accel;}else if(ig.input.state('down')){this.vel.y=accel;}
this.distance+=1*ig.system.tick;this.parent();if(this.pos.y>ig.system.height*0.95){this.pos.y=ig.system.height*0.95;}else if(this.pos.y<ig.system.height*0.15){this.pos.y=ig.system.height*0.15;}},kill:function(){this.parent();},pirateKils:function(amount){this.coins+=amount;},check:function(other){ig.game.spawnEntity(ig.EntityExplosion,this.pos.x,this.pos.y);console.log(this.distance*100);this.kill();}});});

// lib/game/entities/anchor.js
ig.baked=true;ig.module('game.entities.anchor').requires('impact.entity').defines(function(){'use strict';ig.EntityAnchor=ig.Entity.extend({size:{x:12,y:192},offset:{x:10,y:0},vel:{x:0,y:0},maxVel:{x:100,y:100},type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.PASSIVE,animSheet:new ig.AnimationSheet('media/gfx/spr/anchor.png',32,192),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('anchor',1,[0]);this.vel.y=-(Math.random()*100)-10;this.vel.x=-(Math.random()*50);},update:function(){this.parent();if(this.pos.y<-192){this.kill();}}});});

// lib/game/entities/explosion.js
ig.baked=true;ig.module('game.entities.explosion').requires('impact.entity').defines(function(){'use strict';ig.EntityExplosion=ig.Entity.extend({size:{x:90,y:76},maxVel:{x:200,y:200},type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/gfx/spr/explosion.png',90,76),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('explode',0.1,[0,1,2,3,4,5,6,7,8]);ig.game.player=this;},update:function(){this.parent();if(this.currentAnim.frame>=8){this.kill();}}});});

// lib/game/entities/bubble.js
ig.baked=true;ig.module('game.entities.bubble').requires('impact.entity').defines(function(){'use strict';ig.EntityBubble=ig.Entity.extend({size:{x:8,y:5},vel:{x:0,y:-100},maxVel:{x:0,y:200},type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.LITE,animSheet:new ig.AnimationSheet('media/gfx/spr/bubbles.png',8,5),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('float',0.2,[5,4,3,2,1,0]);this.vel.x=Math.random()*50;},update:function(){this.parent();if(this.pos.y<0){this.kill();}}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','game.data.music-manager','game.scenes.ingame','game.scenes.title','plugins.impact-infinite','game.levels.start','game.levels.01','game.levels.02','game.levels.03','game.levels.04','game.levels.05','game.levels.06','game.levels.07','game.levels.08','game.levels.09','game.entities.player','game.entities.anchor','game.entities.explosion','game.entities.bubble').defines(function(){'use strict';ig.launchGame=ig.Game.extend({clearColor:"#d7e894",init:function(){ig.setNocache(true);this.initInput();this.initAudio();},initAudio:function(){ig.music.volume=0;},initInput:function(){gp.registerGamepadEvents();ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.Z,'action_01');ig.input.bind(ig.KEY.X,'action_02');ig.input.bind(ig.KEY.C,'action_03');ig.input.bind(ig.KEY.V,'action_04');},update:function(){ig.system.setGame(ig.Title);}});ig.System.scaleMode=ig.System.SCALE.CRISP;ig.main('#canvas',ig.launchGame,60,160,144,4);});