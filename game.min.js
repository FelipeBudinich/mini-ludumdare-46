...
ig.baked=true;ig.module('game.scenes.ingame').requires('impact.game').defines(function(){'use strict';ig.Ingame=ig.Game.extend({clearColor:"#d7e894",ingameMusic:new ig.Sound('media/snd/msc/ingame.*'),init:function(){this.initMusic();},initMusic:function(){if(ig.music.tracks.length>=1){ig.music.tracks.shift();}
ig.music.add(this.ingameMusic,'ingame');ig.music.play('ingame');},update:function(){this.parent();this.handleInput();},handleInput:function(){if(ig.input.pressed('action_01')||ig.input.pressed('action_02')){ig.system.setGame(ig.Title);return;}},draw:function(){this.parent();}});});

// lib/game/scenes/title.js
ig.baked=true;ig.module('game.scenes.title').requires('impact.game').defines(function(){'use strict';ig.Title=ig.Game.extend({clearColor:"#d7e894",titleMusic:new ig.Sound('media/snd/msc/title.*'),logoImage:new ig.Image('media/logo.png'),logoX:0,logoY:0,backgroundImage_01:new ig.Image('media/title_bg.png'),bkg_01X:0,bkg_01Y:0,backgroundImage_02:new ig.Image('media/title_bg.png'),bkg_02X:144,counter:0,init:function(){this.initMusic();},initMusic:function(){if(ig.music.tracks.length>=1){ig.music.tracks.shift();}
ig.music.add(this.titleMusic,'title');ig.music.play('title');},update:function(){this.parent();this.animateLogo(this.logoX,this.logoY);this.animateBackground();this.handleInput();},handleInput:function(){if(ig.input.pressed('action_01')||ig.input.pressed('action_02')){ig.system.setGame(ig.Ingame);return;}},animateLogo:function(x,y){this.counter+=3*ig.system.tick;this.logoX=Math.cos(this.counter)-4;this.logoY=Math.sin(this.counter);},animateBackground:function(){this.bkg_01X+=6*ig.system.tick;this.bkg_02X=this.bkg_01X-160;if(this.bkg_01X>160){this.bkg_01X=0;}
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','game.data.music-manager','game.scenes.ingame','game.scenes.title').defines(function(){'use strict';ig.launchGame=ig.Game.extend({clearColor:"#d7e894",init:function(){ig.setNocache(true);this.initInput();this.initAudio();},initAudio:function(){},initInput:function(){ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.Z,'action_01');ig.input.bind(ig.KEY.X,'action_02');ig.input.bind(ig.KEY.C,'action_03');ig.input.bind(ig.KEY.V,'action_04');gp.registerGamepadEvents();},update:function(){ig.system.setGame(ig.Title);}});ig.System.scaleMode=ig.System.SCALE.CRISP;ig.main('#canvas',ig.launchGame,60,160,144,4);});