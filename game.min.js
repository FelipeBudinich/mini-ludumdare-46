...
ig.baked=true;ig.module('game.entities.rock').requires('impact.entity','impact.entity-pool').defines(function(){'use strict';ig.EntityRock=ig.Entity.extend({size:{x:4,y:192},offset:{x:15,y:3},vel:{x:0,y:0},maxVel:{x:0,y:0},type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.PASSIVE,animSheet:new ig.AnimationSheet('media/gfx/spr/rock.png',32,192),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('anchor',1,[0]);if(Math.random()>0.5){this.anims.anchor.flip.x=true;}
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','game.data.storage','game.scenes.ingame','game.scenes.title','plugins.impact-infinite','game.levels.start','game.levels.01','game.levels.02','game.levels.03','game.levels.04','game.levels.05','game.levels.06','game.levels.07','game.levels.08','game.levels.09','game.entities.player','game.entities.anchor','game.entities.rock','game.entities.explosion','game.entities.highscore','game.entities.bubble').defines(function(){'use strict';ig.launchGame=ig.Game.extend({clearColor:"#d7e894",init:function(){ig.setNocache(false);this.initInput();this.initAudio();},initAudio:function(){},initInput:function(){gp.registerGamepadEvents();ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.W,'up');ig.input.bind(ig.KEY.S,'down');ig.input.bind(ig.KEY.A,'left');ig.input.bind(ig.KEY.D,'right');},update:function(){ig.system.setGame(ig.Title);}});ig.System.scaleMode=ig.System.SCALE.CRISP;ig.main('#canvas',ig.launchGame,60,160,144,4);});